...
type PatientSortColumn = 'name' | 'aktivCount' | 'otherCount';
...
export default function PatientView() {
  ...
  const [patientSort, setPatientSort] = useState<{ column: PatientSortColumn; direction: SortDirection }>({
    column: 'name',
    direction: 'asc'
  });
  ...
  // Sort patients
  const getSortedPatients = (patients: PatientData[]) => {
    return [...patients].sort((a, b) => {
      let comparison = 0;

      switch (patientSort.column) {
        case 'name':
          comparison = a.name.localeCompare(b.name, 'de');
          break;
        case 'aktivCount':
          comparison = a.aktivCount - b.aktivCount;
          break;
        case 'otherCount':
      ...
      return patientSort.direction === 'asc' ? comparison : -comparison;
    });
  };

  // Handle patient sort column click
  const handlePatientSort = (column: PatientSortColumn) => {
    setPatientSort(prev => {
  ...
  // Patient sort indicator component
  const PatientSortIndicator = ({ column }: { column: PatientSortColumn }) => {
    if (patientSort.column !== column) {
      return <span className="text-gray-400 ml-1">⇅</span>;
    }
    return (
      <span className="ml-1">
        {patientSort.direction === 'asc' ? '▲' : '▼'}
      </span>
    );
  };
      ...
      <main className="container mx-auto px-4 py-8 max-w-[1600px]">
        ...
        {filteredPatients.length === 0 ? (
          ...
          <div className="bg-white rounded-lg border border-border shadow-sm overflow-hidden">
            {/* Patient List Header */}
            <div className="bg-gray-100 px-6 py-3 flex items-center gap-4 border-b border-border sticky top-0">
              <div className="w-6"></div> {/* Spacer for arrow */}

              <button
                onClick={() => handlePatientSort('name')}
                className="flex-1 flex items-center font-semibold text-gray-700 hover:bg-gray-200 px-2 py-1 rounded transition-colors text-left"
              >
                Patient Name
                <PatientSortIndicator column="name" />
              </button>

              <button
                onClick={() => handlePatientSort('aktivCount')}
                className="flex items-center justify-center gap-2 font-semibold text-gray-700 hover:bg-gray-200 px-2 py-1 rounded transition-colors whitespace-nowrap w-[140px]"
              >
                Offene VOs
                <PatientSortIndicator column="aktivCount" />
              </button>

              <button
                onClick={() => handlePatientSort('otherCount')}
                className="flex items-center justify-center gap-2 font-semibold text-gray-700 hover:bg-gray-200 px-2 py-1 rounded transition-colors whitespace-nowrap w-[200px]"
              >
                Abgeschlossene VOs
                <PatientSortIndicator column="otherCount" />
              </button>
            </div>

            {/* Patient Rows */}
            {getSortedPatients(filteredPatients).map((patient) => {
        ...
        {/* Back to Notification Center Link */}
        <div className="mt-8 text-center pb-8">
          <Link
            href="/prototypes/notification-center"
            className="text-primary hover:underline"
          >
            ← Zurück zum Notification Center
          </Link>
        </div>
      </main>
