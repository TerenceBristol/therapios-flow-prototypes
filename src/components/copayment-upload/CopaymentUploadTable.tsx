...
import { CopaymentUploadStatus, Note } from './CopaymentUploadDetailModal';
...
interface CopaymentUploadTableProps {
  uploads: CopaymentUpload[];
  onViewUpload: (upload: CopaymentUpload) => void;
}

export default function CopaymentUploadTable({ uploads, onViewUpload }: CopaymentUploadTableProps) {
  const getStatusBadgeColor = (status: CopaymentUploadStatus) => {
    switch (status) {
      case 'in Prüfung':
  ...
  if (uploads.length === 0) {
    return (
      <div className="bg-white border border-gray-200 rounded-lg p-12 text-center">
        ...
        <p className="mt-1 text-sm text-gray-500">Get started by uploading a copayment slip.</p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="px-4 py-3 text-left font-semibold text-gray-700">Copayment Batch ID</th>
